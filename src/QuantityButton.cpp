...
std::string QuantityButton::STR_BTN_OTHER_FLAVOURS = "Other Soaps";

int QuantityButton::m_Rows = 3;
int QuantityButton::m_Cols = 2;

TgBot::GenericReply::Ptr QuantityButton::prepareMenu(std::map<std::string, std::shared_ptr<AuraButton>>& listAuraBtns, TgBot::Message::Ptr pMsg, FILE *fp) {
	fprintf(fp, "AURA %ld: \"QuantityButton::prepareMenu\" onClick\n", time(0)); fflush(fp);
	...
	TgBot::KeyboardButton::Ptr kbBtnBuySoap, kbBtnVwCart, kbBtnChkOut;
	row.clear();
	kbBtnBuySoap 		= std::make_shared<TgBot::KeyboardButton>();
	kbBtnBuySoap->text	= STR_BTN_OTHER_FLAVOURS;
	listAuraBtns[kbBtnBuySoap->text] = listAuraBtns[StartButton::STR_BTN_BUY_SOAPS];
	row.push_back(kbBtnBuySoap);
...
void QuantityButton::onClick(TgBot::Message::Ptr pMsg, FILE *fp) {
...
void QuantityButton::setEvent(std::string clickMsg, int soapId) {
